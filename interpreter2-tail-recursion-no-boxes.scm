...
(define outer-interpret
  (lambda (file)
    (scheme->language
     (interpret-outer-state (parser file) (newenviornment) (lambda (v) v)
                            (lambda (env) (myerror "Break used outside of loop")) (lambda (env) (myerror "Continue used outside of loop"))
                            (lambda (v env) (myerror "Uncaught exception thrown")) (lambda (env) env)))))

; interprets a list of statements.  The state/environment from each statement is used for the next ones.
(define interpret-statement-list
  (lambda (statement-list environment return function_return break continue throw next)
...
(define interpret-outer-state
  (lambda (statement environment return next)
    (cond
      ...
      ((eq? 'function (statement-type statement)) (interpret-func-def statement environment next))
...
(define interpret-statement
  (lambda (statement environment return function-return break continue throw next)
    (cond
      ...
      ((eq? 'function (statement-type statement)) (interpret-func-def statement enviornment return next))
      ((eq? 'funccall (statement-type statement)) (interpret-func-call statement enviornment return throw next))
      (else (myerror "Unknown statement:" (statement-type statement))))))


;Creating within the environment a binding of (function-name function-closure)
(define interpret-function-def
  (lambda (statement enviornment return)
    (return (create-binding (getFunctionName statement) (create-closure statement enviornment) environment))))

(define getFunctionName cadr)
...
(define create-closure
  (lambda (statement enviornment)
    (list (caddr statement) (cadddr statement) (function-environment enviornment))))
...
(define interpret-func-call
  (lambda (statement envronment return throw next)
    (begin
      (interpret-function-with-return function-body-with-closure bind-parameters-with-new-layer-to-environment))))  

(define bind-parameters
  (lambda (acutal_params formal_params environment return throw next)
    (cond
      ((null? actual_param) (return enviornment))
...
;TODO - DOESNT WORK ATM
;OUTLINE OF FUNCTIONALITY IS WRITTEN OUT
(define eval-func-call
  (lambda (statement environment return throw)
    (letrec ((get-function-closure)
    (cond
      ((eq? function-closure 'error) (myError "Undefined Function"))
      (else (return (get-binding return-called-function-code)))))))) 
