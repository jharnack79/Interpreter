...
(define Mvalue_function_call-cps
  (lambda (expr s return throw class_name instance)
   ...
   ; (display "\n")
    ...
    (cond
      ((eq? function_closure 'error) (error "calling undefined function"))
      ((not(pair? function_closure)) (return instance))
      (else
...
(define Mstate_function_call-cps
  (lambda (expr s return throw class_name instance)
   ...
   ; (display "\n")
   ; (display  s)
    (letrec ((function_class (get_function_class expr s class_name instance)) 
             (function_instance (get_function_instance expr s class_name instance)))
    (begin
      (interpret_parse_tree_return
